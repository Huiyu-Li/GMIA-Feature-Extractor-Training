...
class DenseNet(nn.Module):
    ...
    def forward_split(self, x):
        ...
        for i in range(len(self.block_config)):
            denseblock = getattr(self, f'denseblock{str(i+1)}')
            x = denseblock(x)
            # print(f'mapping_{str(m)}: {x.shape}')
            if i != len(self.block_config) - 1:
                transition = getattr(self, f'transition{str(i+1)}')
                x = transition(x)

        norm5 = getattr(self, 'norm5')
        x = norm5(x)

        # print(f'features: {features.shape}')
        out = F.relu(x, inplace=True) #F.relu(features, inplace=True)
        out = F.adaptive_avg_pool2d(out, (1, 1))
        out = torch.flatten(out, 1)
...
'''
def _densenet(arch, num_in_features, num_init_features, growth_rate, block_config, pretrained, progress,
              **kwargs):
    model = DenseNet(num_in_features, num_init_features, growth_rate, block_config, **kwargs)
    if pretrained:
        _load_state_dict(model, model_urls[arch], progress)
    return model


def DenseNet161(num_in_features, pretrained=False, progress=True, activations='relu', **kwargs):
    ...
    return _densenet('densenet161', num_in_features, 96, 48, (6, 12, 36, 24), pretrained, progress,
                     **kwargs)


def DenseNet169(num_in_features, pretrained=False, progress=True, activations='relu', **kwargs):
    ...
    activation_func = nn.ReLU if activations=='relu' else nn.ELU
    # print (activation_func)
    return _densenet('densenet169', num_in_features, 64, 32, (6, 12, 32, 32), pretrained, progress,
                     **kwargs)


def DenseNet201(num_in_features, pretrained=False, progress=True, activations='relu', **kwargs):
    ...
    activation_func = nn.ReLU if activations=='relu' else nn.ELU
    # print (activation_func)
    return _densenet('densenet201', num_in_features, 64, 32, (6, 12, 48, 32), pretrained, progress,
                     **kwargs)
